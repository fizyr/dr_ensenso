...
void toCvMat(
	...
	if (roi && !roi->empty()) {
		if (height != std::size_t(roi->height) || width != std::size_t(roi->width)) {
			throw std::runtime_error("ROI height: " + std::to_string(roi->height) + " and width: " + std::to_string(roi->width) + " differs from input height: " + std::to_string(height) + " and width: " + std::to_string(width) + ".");
		}
