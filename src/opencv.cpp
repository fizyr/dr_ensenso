...
Result<cv::Mat> toCvMat(NxLibItem const & item, std::optional<cv::Rect> roi, std::string const & what) {
	int error = 0;
	cv::Mat result;
...
Result<void> toCvMat(
	...
	if (roi && !roi->empty()) {
		if (height != std::size_t(roi->height) || width != std::size_t(roi->width)) {
			return estd::error("ROI height: " + std::to_string(roi->height) + " and width: " + std::to_string(roi->width) + " differs from input height: " + std::to_string(height) + " and width: " + std::to_string(width));
		}
...
Result<cv::Mat> toCameraMatrix(NxLibItem const & item, std::string const & camera, std::string const & what) {
	int error = 0;
...
Result<cv::Mat> toDistortionParameters(NxLibItem const & item, std::string const & camera, std::string const & what) {
	int error = 0;
...
Result<cv::Mat> toRectificationMatrix(NxLibItem const & item, std::string const & camera, std::string const & what) {
	int error = 0;
	cv::Mat result = cv::Mat::zeros(3, 3, CV_64F);
	for (std::size_t i = 0; i < 3; i++) {
		for (std::size_t j = 0; j < 3; j++) {
...
Result<cv::Mat> toProjectionMatrix(NxLibItem const & item, std::string const & camera, std::string const & what) {
	int error = 0;
